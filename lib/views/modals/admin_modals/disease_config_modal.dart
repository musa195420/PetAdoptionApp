...
class DiseaseConfigModal extends StatelessWidget {
  DiseaseConfigModal({super.key});
  ...
  Widget _buildAddAnimal(GeneralConfigViewModel viewModel) {
     animalController=TextEditingController(text: viewModel.animalName);
       diseaseController=TextEditingController(text: viewModel.diseaseName);
        ...
        child: Column(
          ...
          if(viewModel.selectedIndex==0)
            DefaultTextInput(
              ...
              validator: (value) {
                if (value == null || value.isEmpty) {
                  return 'Enter Disease  Name Please';
                }
                return null;
              },
            ),
            ...
            DefaultTextInput(
              controller: diseaseController,
              labelText: "Add Disease  Names In Bulk",
              hintText: "Separate By Commas For Example Feline leukemia, Feline infectious peritonitis",
              maxLines: 5,
              icon: Icons.pets_outlined,
              validator: (value) {
                if (value == null || value.isEmpty) {
                  return 'Enter Disease  animal names';
                }
                return null;
              },
            ),
             ...
             DefaultTextInput(
              ...
              hintText: "Add Disease Description",
              maxLines: 3,
              icon: Icons.description,
             
            ),
          ],)),
            _buildButton(viewModel, "Add Disease", () {
              if (formKey2.currentState!.validate()) {
                viewModel.diseaseName = diseaseController.text;
                
                viewModel.description = descriptionController.text;
               viewModel.addDiseases(diseaseController.text,descriptionController.text);
              }
            }),
          ],
        ),
 ...
 descriptionController = TextEditingController(text: viewModel.description??"");
        ...
        child: Column(
          ...
          children: [
            ...
            DefaultTextInput(
              ...
              hintText: "Disease Name",
              icon: Icons.pets_outlined,
              validator: (value) => value == null || value.isEmpty ? 'Disease Name required' : null,
            ),
            ...
            _buildButton(viewModel, "Update Disease", () {
              if (formKey.currentState!.validate()) {
                viewModel.diseaseName = diseaseController.text;
                viewModel.description = descriptionController.text;
                viewModel.updateDisease(diseaseController.text,descriptionController.text);
              }
            }),
          ],
        ),
  ...
  return Column(
    children: [
            ...
            Expanded(
              child: Padding(
                ...
                child: TextField(
                  ...
                  onChanged: viewModel.filteredDiseases,
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                onTap: () {
                  viewModel.filteredSelectionDisease(animal);
                },
                child: Container(
      ...
      Expanded(
        child: viewModel.filteredDisease == null || viewModel.filteredDisease!.isEmpty
            ? const Center(child: Text("No Animals loaded."))
            : ListView.builder(
                padding: const EdgeInsets.all(2),
                itemCount: viewModel.filteredDisease!.length,
                itemBuilder: (context, index) {
                  final disease = viewModel.filteredDisease![index];
                  return _buildUserCard(context, disease, viewModel);
                },
              ),
      ),
    ],
  ...
  Widget _buildUserCard(BuildContext context, Disease disease, GeneralConfigViewModel viewModel) {
    return Card(
  ...
  title: Row(
    ...
    children: [
      ...
      child:Text(disease.animal??"",style: TextStyle(color: Colors.white,fontWeight: FontWeight.w400),) ,
      ),
      Flexible(child: Text(disease.name??"")),
    ],
  ),
  trailing: Row(
    ...
    children: [
      ...
      IconButton(
        ...
        onPressed: () {
          viewModel.gotoEditDiseases(disease);
        },
      ),
       IconButton(
        icon: const Icon(Icons.delete, color: Colors.red),
        onPressed: () {
          viewModel.deleteDisease(disease.diseaseId);
        },
      )
    ],
  ),
