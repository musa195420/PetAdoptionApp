...
class SecuremeetupAdminViewModel
...
Color getColor(String approval)
{
  ...
  void filterSecureMeetup(String pattern) {
    if (pattern.trim().isEmpty) {
      ...
      filteredSecure = secureMeetups
          ?.where((u) => u.currentAddress!.toLowerCase().contains(pattern.toLowerCase()))
          .toList();
    }
    notifyListeners();
  }
 ...
 Future<void> deleteSecureMeetup(String secureId) async {
   try{
    ...
    bool res=await _dialogService.showAlertDialog(Message(description:"Do you Really want to delete SecureMeetup ?"));
   if(res)
   {
   var resDelete= await _apiService.deleteSecureMeetup(SecureMeetup(secureMeetupId:secureId) );
  ...
  void updateSecureMeetup(String phoneNumber, String currentAddress,String rejectionReason,SecureMeetup secure)async {
   ...
   try{
    ...
    loading(true,loadingText: "Secure Meetup Updating ...");
     ...
     var updateRes=await _apiService.updateSecureMeetup(
      SecureMeetup(
         secureMeetupId:secure.secureMeetupId,  meetupId:secure.meetupId,  phoneNumber:phoneNumber,  currentAddress:currentAddress,rejectionReason: rejectionReason,approval: meetup!.approval??"Pending" )
...
debugPrint(e.toString());
  ...
  try{
     ...
     if(adopterIdBackPath!=null || adopterIdPath!=null ||proofPicPath!=null)
   {
    var res= await _apiService.uploadSecureMeetupImages(ProofImages(meetupId: meetupId,proofPicPath: proofPicPath,adopterIdFrontPath: adopterIdPath,adopterIdBackPath: adopterIdBackPath));
...
SecureMeetup? meetup;
  ...
  bool isEdit=false;
  void setMeetup(SecureMeetup meetup) {
    this.meetup=meetup;
  }
